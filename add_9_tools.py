...
"""
    if "nav-jwt-decoder" not in html:
        # Inject right before the legal navigation items
        html = html.replace('<!-- Legal & Support -->', nav_inject + '\n                <!-- Legal & Support -->')
...
"""
    if "jwt-decoder" not in html.split('id="panel-dashboard"')[1].split('</section>')[0]:
        # Inject right before the closing div of tool-grid
        html = html.replace('                    </div>\n                </section>\n\n                <!-- TAB 2: QR GENERATOR -->', dashboard_cards + '                    </div>\n                </section>\n\n                <!-- TAB 2: QR GENERATOR -->')
                    ...
                    <div class="panel-content glass-card" style="padding: 1rem;">
                        <div class="grid-2" style="gap: 1rem; height: 500px;">
                            <div class="form-group" style="margin-bottom: 0; display: flex; flex-direction: column;">
                                ...
                                <textarea id="markdownInput" placeholder="# Hello World..." oninput="renderMarkdown()" style="flex: 1; resize: none; font-family: monospace;"></textarea>
                            </div>
                            <div class="form-group" style="margin-bottom: 0; display: flex; flex-direction: column;">
...
"""
    if "panel-jwt-decoder" not in html:
        # Insert right before the blog panel
        html = html.replace('<!-- TAB 20: BLOG HUB -->', panels_html + '\n                <!-- TAB 20: BLOG HUB -->')

    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print("9 new tool panels injected into index.html successfully.")
